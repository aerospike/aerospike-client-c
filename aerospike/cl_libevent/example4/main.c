...
void
example_phase_two_a(int return_value, evcitrusleaf_bin *bins, int n_bins, uint32_t generation, void *udata)
{
	config *c = (config *) udata;
	...
	evtimer_set(&c->ev, example_phase_two_b, c);
...
example_phase_one(config *c)
{
	...
	int rv = evcitrusleaf_put(c->asc, c->ns, c->set, &c->o_key, values, 1, &wparam, c->timeout_ms, example_phase_two_a, c);
	if (rv != 0) {
