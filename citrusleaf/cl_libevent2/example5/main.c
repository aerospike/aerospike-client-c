...
example5_request(uint64_t i)
{
	...
	ev2citrusleaf_object_init_int(&req->o_key, i);
	
	int rv = ev2citrusleaf_get_all(c->asc, c->ns, c->set, &req->o_key, c->timeout_ms, example5_response, (void *) i, c->base);
	if (rv != 0) {
...
main(int argc, char **argv)
{
	...
	event_base_loopexit ( g_config.base, &le_tv );   	
