...
class LuaModulelevel(LuaObject):
    ...
    def get_index_text(self, modname, name_cls):
        if self.objtype == 'function':
            if not modname:
                return _('%s() (Lua function)') % name_cls[0]
            return _('%s() (Lua function)') % (name_cls[0])
        elif self.objtype == 'data':
            if not modname:
                return _('%s (Lua variable)') % name_cls[0]
            return _('%s (Lua variable)') % (name_cls[0])
        else:
            return ''
...
class LuaClasslike(LuaObject):
    ...
    def get_index_text(self, modname, name_cls):
        if self.objtype == 'class':
            if not modname:
                return _('%s (Lua class)') % name_cls[0]
            return _('%s (Lua class in %s)') % (name_cls[0], modname)
    ...
    def get_index_text(self, modname, name_cls):
        ...
        if self.objtype == 'method':
            try:
                ...
                if modname:
                    return _('%s() (Lua method)') % (name)
                else:
                    return '%s()' % name
            if modname and add_modules:
                return _('%s (Lua method)') % (methname)
            else:
                return _('%s (Lua method)') % (methname)
        else:
            return _('%s (Lua class)') % name_cls[0]
...
class LuaModule(Directive):
    ...
    def run(self):
        ...
        if not noindex:
            ...
            indextext = _('%s (Lua module)') % modname
