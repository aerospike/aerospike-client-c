...
extern int aerospike_lset_create(
        cl_cluster * asc, char * namespace, char * set, char * keystr,
        char * lset_bin_name, int distribution,
        char * lso_package, uint32_t timeout_ms);

extern int aerospike_lset_insert(
        cl_cluster * asc, char * ns, char * set, char * keystr,
        char * lset_bin_name, as_val * lset_valuep, char * lso_package,
        uint32_t timeout_ms );

extern as_result * aerospike_lset_search(
        cl_cluster * asc, char * ns, char * set, char * keystr,
        char * lset_bin_name, as_val * search_valuep, bool exists,
        char * lso_package, uint32_t timeout_ms);

extern int aerospike_lset_delete(
        cl_cluster * asc, char * ns, char * set, char * keystr,
        char * lset_bin_name, as_val * delete_valuep, char * lso_package,
        uint32_t timeout_ms);

#if 0
//NOTE: (REMOVED_FROM: BETA_3.0) QA pass (ADDTO: BETA_3.1)
extern int aerospike_lset_insert_with_transform(cl_cluster * asc, char * ns,
                                  char * set,
...
extern as_result * aerospike_lset_search_with_transform(cl_cluster * asc, char * ns,
