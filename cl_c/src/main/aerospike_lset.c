 ...
 */
int
aerospike_lset_create( cl_cluster * asc, char * namespace, char * set,
  ...
  // Lua:asLSetCreate(record, NS, Set, SBin, Distrib)
	...
	char * udf_function_name = "aerospike_lset_create";
 ...
 */
int
aerospike_lset_insert(cl_cluster * asc, char * ns, char * set, char * keystr,
			   char * lset_bin_name, as_val * lset_valuep, 
			   char * lso_package, uint32_t timeout_ms ) {
	static char * meth = "aerospike_lset_insert()";
	int rc = 0; // ubiquitous return code
	char * udf_function_name = "aerospike_lset_insert"; // The Lua Function we'll call
 ...
 */
int
aerospike_lset_insert_with_transform(cl_cluster * asc, char * ns, char * set,
	...
	static char * meth = "aerospike_lset_insert_with_transform()";
	int rc = 0;

	// TO BE FILLED IN SHORTLY  -- when we start testing set value compression.

	return rc;
} // end aerospike_lset_insert_with_transform()
 ...
 */
as_result  *
aerospike_lset_search(cl_cluster * asc, char * ns, char * set, char * keystr,
		char * lset_bin_name, as_val * search_valuep, bool exists,
		char * lso_package, uint32_t timeout_ms) {
	static char * meth = "aerospike_lset_search()";
	int rc = 0; // ubiquitous return code
	char * udf_function_name = "aerospike_lset_search";
...
/** ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
 *  aerospike_lset_search_with_transform()
 ...
 */
as_result  *
aerospike_lset_search_with_transform(cl_cluster * asc, char * ns, char * set,
							  char * keystr, char * lset_bin_name,
							  as_val * search_valuep, bool exists,
							  char * lso_package, char * udf_file,
							  char * udf_name, as_list * function_args,
							  uint32_t timeout_ms ) {
	static char * meth = "aerospike_lset_search_with_transform()";
	int rc = 0;
	as_result * resultp = NULL;
	...
	return resultp;
} // end aerospike_lset_search_with_transform()
 ...
 */
int
aerospike_lset_delete(cl_cluster * asc, char * ns, char * set, char * keystr,
			   char * lset_bin_name, as_val * delete_valuep,
			   char * lso_package, uint32_t timeout_ms ) {
	static char * meth = "aerospike_lset_delete()";
	int rc = 0; // ubiquitous return code
	char * udf_function_name = "aerospike_lset_delete";
