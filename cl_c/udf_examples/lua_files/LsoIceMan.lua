...
-- TODO: Verify correct recovery order for operations.
-- ======================================================================
-- Aerospike Calls:
...
-- that the underlying CREC operations have succeeded.
-- ======================================================================
...
local function   warmListChunkCreate( topRec, lsoMap )
  ...
  -- after all of the underlying CREC operations have been done.
...
local function coldDirHeadCreate( topRec, lsoMap )
  ...
  -- Create the Aerospike Record, initialize the bins: Ctrl, List
  -- Note: All Field Names start with UPPER CASE.
  local newColdHead = aerospike:crec_create( topRec );
...
-- ======================================================================
-- coldDirRecInsert(coldHeadRec,digestListIndex,digestList);
-- ======================================================================
...
-- ======================================================================
local function coldDirRecInsert( coldHeadRec, digestListIndex, digestList )
  local mod = "LsoIceMan";
...
--  GP=F and trace("[ENTER]: <%s:%s> ", mod, meth );
  ...
  then
    ...
    stringDigest = tostring( coldHeadDigest );
    coldHeadRec = aerospike:crec_open( topRec, stringDigest );
  end
  ...
  while digestsLeft > 0 do
    digestsWritten = coldDirRecInsert(coldHeadRec,digestListIndex,digestList);
    ...
    if digestsLeft > 0 then
      aerospike:crec_update( topRec, coldHeadRec );
      aerospike:crec_close( topRec, coldHeadRec );
  ...
  local status = aerospike:crec_update( topRec, coldHeadRec );
  GP=F and trace("[DEBUG]: <%s:%s> CREC Update Status(%s) ",
    mod,meth, tostring(status));

  status = aerospike:crec_close( topRec, coldHeadRec );
  GP=F and trace("[DEBUG]: <%s:%s> CREC Close Status(%s) ",
    mod,meth, tostring(status));
...
local function coldListRead(topRec, resultList, lsoMap, count,
  ...
  while dirPageDigest ~= nil and dirPageDigest ~= 0 do
    ...
    local dirPageRec = aerospike:crec_open( topRec, stringDigest );
    ...
    if countRemaining <= 0 or dirPageCtrlMap.NextDirRec == 0 then
        ...
        aerospike:crec_close( topRec, dirPageRec );
        return totalNumRead;
    end
...
-- Parms:
-- (*) topRec: The top level user record (needed for crec_create)
-- (*) lsoMap
...
local function warmListTransfer( topRec, lsoMap )
  ...
  info("[WOO HOO!!]: <%s:%s> >>>00000000000000000000000000<<<", mod, meth );
  info("[WOO HOO!!]: <%s:%s> >>> USING THE COLD LIST !!!! <<<", mod, meth );
  info("[WOO HOO!!]: <%s:%s> >>>00000000000000000000000000<<<", mod, meth );
  info("[DEBUG]: <%s:%s> >>>>>>> TRANSFER WARM TO COLD<<<<<<:: Map(%s)",
    mod, meth, tostring(lsoMap) );
