...
local function extractTransferList( lsoMap )
  ...
  -- Three separate Debug prints because one print can deliver only so much
  info("[DEBUG]: <%s:%s>OldHotCache(%s)\n",mod,meth,tostring(oldHotCacheList));
  info("[DEBUG]: <%s:%s>NewHotCache(%s)\n",mod,meth,tostring(oldHotCacheList));
  info("[DEBUG]: <%s:%s>ResultList(%s)\n", mod,meth, tostring(resultList));
...
local function ldrChunkInsert(topWarmChunkRecord,lsoMap, listIndex, insertList )
  ...
  info("[DEBUG]: <%s:%s>: Copying From(%d) to (%d) Amount(%d)List(%s)\n",
  ...
  for i = 0, (newItemsStored - 1), 1 do
    list.append( ldrValueList, insertList[listIndex + i] );
...
local function   warmDirListChunkCreate( topRec, lsoMap )
  ...
  -- TODO: Reset entry max to 100
  ctrlMap.EntryMax = 10; -- Move up to TopRec -- when Stable
  ...
  -- Two prints to get ALL the info thru.
  info("[DEBUG]: <%s:%s> Post CHunkAppend:NewChunk(%s):\n",
    mod, meth, tostring(newChunkDigest) );
  info("[DEBUG]: <%s:%s> Post CHunkAppend:LsoMap(%s)\n",
    mod, meth, tostring(lsoMap));
...
-- ======================================================================
-- warmDirRead(topRec, resultList, lsoMap, Count, func, fargs, all);
-- ======================================================================
-- Synopsis:
-- Parms:
...
--info("[ENTER]: <%s:%s> LSO Summary(%s) \n", mod, meth, lsoSummary(lsoMap) );
  ...
  if list.size( warmDirList ) == 0 or lsoMap.WarmTopFull == 1 then
...
local function lsoMapRead( topRec, lsoMap, peekCount, func, fargs )
  ...
  -- If the cache had all that we need (and get ALL not set) then done.
  -- Return list.
  ...
  return resultList; -- No Cold List processing for now.
...
local function initializeLsoMap( topRec, lsoBinName )
  ...
  -- TODO: Reset ENTRY MAX to 100
  lsoMap.LdrEntryMax = 10;  -- Max # of items in a Data Chunk (List Mode)
