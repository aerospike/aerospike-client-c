...
void
example_phase_eight(int return_value, evcitrusleaf_bin *bins, int n_bins, uint32_t generation, void *udata)
{
	fprintf(stderr, "example phase 8 received\n");
	
	if (return_value != EVCITRUSLEAF_FAIL_GENERATION) {
		fprintf(stderr, "example has FAILED? stage 8 return value %d should be %d\n",
			return_value, EVCITRUSLEAF_FAIL_GENERATION);
		test_terminate(-1);
		return;
	}
...
void
example_phase_seven(int return_value, evcitrusleaf_bin *bins, int n_bins, uint32_t generation, void *udata)
{
	...
	if (strcmp(bins[0].bin_name,"test_bin_blob") != 0) {
		fprintf(stderr, "phase 7: name of bin returned is wrong, should be test-bin_blob is %s\n",bins[0].bin_name);
		test_terminate(-1);
		return;
	}
	...
	evcitrusleaf_object_init_blob(&values[0].object, c->blob, c->blob_size);
	...
	if (0 != evcitrusleaf_put(c->asc, c->ns, c->set, &c->o_key, values, 1, 
		&wparam, 0, example_phase_eight, c)) {
...
void
example_phase_six(int return_value, evcitrusleaf_bin *bins, int n_bins, uint32_t generation, void *udata)
{
	config *c = (config *) udata;
	...
	if (strcmp(bins[0].bin_name,"test_bin_blob") != 0) {
		fprintf(stderr, "phase 6: name of bin returned is wrong, should be test_bin_blob, is %s\n",bins[0].bin_name);
		test_terminate(-1);
		return;
	}
	if (bins[0].object.type != CL_BLOB) {
	...
	evcitrusleaf_operation ops[3];
	
	strcpy(ops[0].bin_name, "test_bin_zulu");
	ops[0].op = CL_OP_WRITE;
	evcitrusleaf_object_init_str(&ops[0].object, "yodel!yodel!");
	
	strcpy(ops[1].bin_name, "test_bin_two");  // an overwrite!
	ops[1].op = CL_OP_WRITE; 
	evcitrusleaf_object_init_int(&ops[1].object, 2);
	...
	if (0 != evcitrusleaf_operate(c->asc, c->ns, c->set, &c->o_key, ops, 3, &wparam, c->timeout_ms, example_phase_seven, c)) {
...
void
example_phase_five(int return_value, evcitrusleaf_bin *bins_ignore, int n_bins_ignore, uint32_t generation,void *udata)
{
	config *c = (config *) udata;
	...
	if (bins_ignore)		evcitrusleaf_bins_free(bins_ignore, n_bins_ignore);
	
	const char *bins[1] = { "test_bin_blob" }; 
	
	if (0 != evcitrusleaf_get(c->asc, c->ns, c->set, &c->o_key, bins, 1, c->timeout_ms, example_phase_six, c)) {
...
void
example_phase_four(int return_value, evcitrusleaf_bin *bins, int n_bins, uint32_t generation, void *udata)
{
	config *c = (config *) udata;
	...
	if (0 != evcitrusleaf_put(c->asc, c->ns, c->set, &c->o_key, values, 1, 
		&wparam, c->timeout_ms, example_phase_five, c)) {
...
example_phase_three(int return_value, evcitrusleaf_bin *bins, int n_bins, uint32_t generation, void *udata)
{
	...
	evcitrusleaf_write_parameters wparam;
	evcitrusleaf_write_parameters_init(&wparam);
	if (0 != evcitrusleaf_delete(c->asc, c->ns, c->set, &c->o_key, &wparam,
			c->timeout_ms, example_phase_four, c)) {
...
example_phase_one(config *c)
{
	...
	// to strings
	evcitrusleaf_object_init_str(&c->o_key, "example_key");
	...
	if (0 != evcitrusleaf_put(c->asc, c->ns, c->set, &c->o_key, values, 2, &wparam, c->timeout_ms, example_phase_two, c)) {
...
main(int argc, char **argv)
{
	...
	if (g_config.follow == false)
		evcitrusleaf_cluster_follow(g_config.asc, false);
	...
	int tries = 0;
	do {
		node_count = evcitrusleaf_cluster_get_active_node_count(g_config.asc);
	...
	evcitrusleaf_info(g_config.host, g_config.port, 0, g_config.timeout_ms, example_info_fn, 0);
