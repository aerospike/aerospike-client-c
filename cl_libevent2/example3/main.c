...
example_phase_one(config *c)
{
	...
	int rv = ev2citrusleaf_put(c->asc, c->ns, c->set, &c->o_key, values, 4, &wparam, g_config.timeout_ms, example_phase_two, c);
	if (rv != 0) {
...
example_phase_one(config *c)
{
	...
	int rv = ev2citrusleaf_put(c->asc, c->ns, c->set, &c->o_key, values, 1, &wparam, c->timeout_ms, example_phase_two, c, c->base);
	if (rv != 0) {
		fprintf(stderr, "citrusleaf put failed: error code %d\n",rv);
		test_terminate(rv);
		return;
