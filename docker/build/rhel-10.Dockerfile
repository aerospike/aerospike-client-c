...
RUN wget -q https://ftp.gnu.org/gnu/bison/bison-3.7.4.tar.gz && \
    ...
    cd bison-3.7.4 && ./configure --prefix=/usr/local && make && make install
RUN wget -q https://github.com/westes/flex/releases/download/v2.6.4/flex-2.6.4.tar.gz && \
    tar -xzf flex-2.6.4.tar.gz && \
    cd flex-2.6.4 && ./configure --prefix=/usr/local && make && make install
...
# ubi10-minimal does not include zlib
