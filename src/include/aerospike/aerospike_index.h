...
/**
 * Create secondary index on an expression with a complex datatype.
 *
 * This asynchronous server call will return before the command is complete.
 * The user can optionally wait for command completion by using a task instance.
 *
 ...
 *
 * as_index_task task;
 * if (aerospike_index_create_exp_and_cdt(&as, &err, &task, NULL, "test", "demo",
 *     "idx_test_demo_bin1", AS_INDEX_TYPE_DEFAULT, AS_INDEX_NUMERIC, exp) == AEROSPIKE_OK) {
 *     // TODO: FIX EXAMPLE CODE TO MATCH API.
 *     aerospike_index_create_wait(&err, &task, 0);
 * }
 * as_exp_destroy(exp);
 * @endcode
 ...
 */
AS_EXTERN as_status
aerospike_index_create_exp_and_cdt(
	...
	as_index_type itype, as_index_datatype dtype, struct as_exp* exp, struct as_cdt_ctx* ctx
	);
