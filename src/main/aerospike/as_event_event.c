...
static void
as_event_callback(evutil_socket_t sock, short events, void *user_data);

static void
as_event_close_loop(as_event_loop* event_loop)
{
	if (event_del(&event_loop->wakeup) == -1) {
        as_log_error("as_event_close_loop: event_del failed");
    }
...
static void
as_event_wakeup(evutil_socket_t sock, short events, void *user_data)
{
	// Read command pointers from queue.
	as_event_loop* event_loop = user_data;
	void* cmd;
...
as_event_init_loop(as_event_loop* event_loop)
{
	...
	if (-1 == evthread_make_base_notifiable(event_loop->loop)) {
        as_log_error("as_event_init_loop: evthread_make_base_notifiable failed");
        return;
    }
	
	event_assign(&event_loop->wakeup, event_loop->loop, -1, EV_PERSIST | EV_READ, as_event_wakeup, event_loop);
	if (-1 == event_add(&event_loop->wakeup, NULL)) {
...
as_event_watch(as_event_command* cmd, int watch)
{
	...
	if (event_del(&conn->watcher) == -1) {
        as_log_error("as_event_watch_write: event_del failed");
	}
    ...
    if (event_add(&conn->watcher, NULL) == -1) {
        as_log_error("as_event_watch_write: event_add failed");
	}
}
...
static void
as_event_callback(evutil_socket_t sock, short revents, void *user_data)
{
	if (revents & EV_READ) {
		as_event_connection* conn = user_data;
...
static void
as_event_timeout_cb(evutil_socket_t sock, short events, void *user_data)
{
    // One-off timers are automatically stopped by libevent.
	as_event_timeout(user_data);
}
...
as_event_command_begin(as_event_command* cmd)
{
	...
	if (cmd->timeout_ms) {
		evtimer_assign(&cmd->timer, cmd->event_loop->loop, as_event_timeout_cb, cmd);
		...
		if (evtimer_add(&cmd->timer, &tv) == -1) {
            as_log_error("as_event_command_begin: evtimer_add failed");
		}
	}
...
__attribute__((constructor)) void library_init()
{
    if (-1 == evthread_use_pthreads()) {
        as_log_error("library_init: evthread_use_pthreads failed");
        abort();
    }
}
