...
bool
as_operations_select_by_path(as_operations* ops, const char* name, as_cdt_ctx* ctx, as_cdt_select_flags flags)
{
	if (ctx == NULL) {
		return false;
	}
...
bool
as_operations_modify_by_path(as_operations* ops, const char* name, as_cdt_ctx* ctx, as_exp* mod_exp, as_cdt_modify_flags flags)
{
	if (ctx == NULL) {
		return false;
	}
