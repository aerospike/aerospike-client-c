...
static bool
as_record_rec_destroy(as_rec* r)
{
...
static uint32_t
as_record_rec_hashcode(const as_rec* r)
{
...
static as_val*
as_record_rec_get(const as_rec* r, const char* name)
{
	return r && name ? (as_val *) as_record_get((as_record *) r, name) : NULL;
}

static int
as_record_rec_set(const as_rec* r, const char* name, const as_val * value)
{
	return r && name ? as_record_set((as_record *) r, name, (as_bin_value *)value) : 1;
}

static int
as_record_rec_remove(const as_rec* r, const char* name)
{
	return r && name ? !as_record_set_nil((as_record *) r, name) : 1;
}

static uint32_t
as_record_rec_ttl(const as_rec* r)
{
	return r ? ((as_record *) r)->ttl : 0;
}

static uint16_t
as_record_rec_gen(const as_rec* r)
{
	return r ? ((as_record *) r)->gen : 0;
}

static as_bytes*
as_record_rec_digest(const as_rec* r)
{
	return r ? as_bytes_new_wrap(((as_record *) r)->key.digest.value, AS_DIGEST_VALUE_SIZE, false) : NULL;
}

static uint16_t
as_record_rec_numbins(const as_rec* r)
{
	return r ? as_record_numbins((as_record *) r) : 0;
}

static bool
as_record_rec_foreach(const as_rec* r, as_rec_foreach_callback callback, void* udata)
{
